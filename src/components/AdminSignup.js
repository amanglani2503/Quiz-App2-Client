...
export default function AdminSignup() {
    ...
    const handleSignupSubmit = (e) =>{
      e.preventDefault();
      axios.post('http://localhost:5000/admin/signup', {email : supEmail, password : supPassword, cnfPassword : cnfPassword})
      .then(res =>{
        console.log(res)
        if(res.status === 201){
      ...
      <div className="wrapper">
        ...
        <div className="form-container">
          <div className="slide-controls">
            <Link to="/admin/login" style={linkstyle} className={`slide login ${activeForm === 'login' ? 'active' : ''}`} onClick={() => handleSlideChange('login')}>Login</Link>
            <Link to="/admin/signup" style={linkstyle} className={`slide signup ${activeForm === 'signup' ? 'active' : ''}`} onClick={() => handleSlideChange('signup')}>Signup</Link>
